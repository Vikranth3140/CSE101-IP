...
frnd_addressbook = 'friend_addressbook.txt'
import json

def Menu():
    print("0. Merge address book")
    ...
    while True:
        ...
        if ch==0:
            addressbook = frnd_merge(addressbook)
        elif ch == 1:
            insert(addressbook)
        elif ch == 2:
            delete(addressbook)
...
def frnd_merge(addressbook):
    f = open("addressbook.txt",'r')
    my_load = json.load(f)
    f.close()
    g = open('friend_addressbook.txt','r')
    frnd_load = json.load(g)
    g.close()
    addressbook = merge(my_load,frnd_load)
    return addressbook
